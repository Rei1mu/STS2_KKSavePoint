#!/usr/bin/env bash
# Shared .env loader for ExampleMod bash scripts.

if [[ -n "${EXAMPLEMOD_ENV_LOADED:-}" ]]; then
  return 0
fi

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"
ENV_FILE="${EXAMPLEMOD_ENV_FILE:-${PROJECT_ROOT}/.env}"
...
export EXAMPLEMOD_ENV_FILE="${ENV_FILE}"
export EXAMPLEMOD_ENV_LOADED=1
