...
public static class RollbackMessageHandler
{
    ...
    public static bool SendRollbackMessageWithAck(string savePath, int timeoutMs = 5000)
    {
        try
        {
            ...
            Log.Info($"[KKSavePoint] {clientCount} clients connected, waiting for their acknowledgment...");

            // 如果没有客户端，直接返回成功
            if (clientCount == 0)
            {
                Log.Info("[KKSavePoint] No clients connected, skipping acknowledgment wait");
                SendRollbackMessageInternal(savePath, netService);
                return true;
            }
            ...
            Log.Info("[KKSavePoint] Rollback ack callback set successfully");

            // 发送回滚消息
            SendRollbackMessageInternal(savePath, netService);

            // 等待所有客户端确认
            Log.Info("[KKSavePoint] Waiting for client acknowledgments...");
            bool allAcked = _ackWaitEvent.Wait(timeoutMs);

            if (allAcked)
            {
                Log.Info($"[KKSavePoint] All {_receivedAcks.Count} clients acknowledged rollback request");
            }
            else
            {
                Log.Warn($"[KKSavePoint] Timeout waiting for acknowledgments. Received {_receivedAcks.Count}/{_expectedAckCount}");
            }
    ...
    public static bool SendRollbackMessage(string savePath)
    {
        try
        {
            ...
            return SendRollbackMessageInternal(savePath, netService);
        }
        catch (Exception ex)
        {
    ...
    private static bool SendRollbackMessageInternal(string savePath, object netService)
    {
